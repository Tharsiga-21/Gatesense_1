...
export function generateLocalReasoning(query: string, gates: GateData[]): QueryResponse {
  ...
  // Localized sustainability notes
  const sustainabilityNotes: Record<string, string> = {
    ...
    hi: "🌱 ग्रीन टूर्नामेंट: विश्व कप 2026 स्थिरता का समर्थन करें! गेट D के लिए CDMX मेट्रो लाइन 3 या गेट F के लिए मेट्रोबस लाइन 1 लें। गेट A और E के पास पर्यावरण अनुकूल कचरा वर्गीकरण केंद्र सक्रिय हैं।"
  };

  // Edge Case 1: Empty or very short/gibberish input
  if (normalizedQuery.length < 3 || /^[^\w\s\u0600-\u06FF\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF\uAC00-\uD7AF\u0900-\u097F]+$/.test(normalizedQuery)) {
    const errorResponses: Record<string, { r: string, msg: string }> = {
      ...
      pt: { r: "D", msg: "Sua consulta parece vazia ou não está clara. Por favor, forneça detalhes sobre sua localização atual (ex: 'perto do Portão B') e sua seção ou assento de destino para que possamos calcular a recomendação de portão mais segura e eficiente." },
      ...
      ja: { r: "D", msg: "クエリが不明確です。安全で最も効率的なゲート推薦を計算できるよう、現在の場所（例：ゲートB付近）や目的のセクションなどの詳細を入力してください。" },
  ...
  const gateMatch = normalizedQuery.match(/gate\s*([a-f])\b/) || 
  ...
  if (gateMatch) {
    // If Chinese match structure "A号门" is used, the letter might be captured in index 1
  ...
  if (isAccessible) {
    // Look at Gates C and D. Find whichever has lower density.
    ...
    // If at least one of C or D is under the HIGH_CONGESTION_THRESHOLD, recommend it even if it's slightly busier than the absolute best gate.
  ...
  if (mentionedGate) {
    ...
    if (currentDensity > HIGH_CONGESTION_THRESHOLD) {
      ...
      const responses: Record<string, string> = {
        ...
        ar: `البوابة ${mentionedGate} مزدحمة للغاية حالياً (سعة ${currentDensity}%). ننصح بشدة بالتوجه إلى البوابة ${altLetter} البديلة والأقل ازدحاماً (سعة ${alternativeGate.density}%) لتوفير الوقت والجهد وتجنب طوابير الانتظار الطويلة.`,
        ja: `現在ゲート${mentionedGate}は非常に混雑しています（混雑度${currentDensity}%）。代わりに混雑度がわずか${alternativeGate.density}%のゲート${altLetter}を使用することを強くお勧めします。行列が少なく、スムーズに入場可能です。`,
        zh: `${mentionedGate} 号门当前极度拥堵（拥堵度 ${currentDensity}%）。我们强烈建议您改用 ${altLetter} 号门，那里的混杂度仅为 ${alternativeGate.density}%。排队人数极少，能让您更快、更安全地入场就座。`,
        ko: `현재 게이트 ${mentionedGate}은 매우 혼잡합니다(혼잡도 ${currentDensity}%). 대신 대기열이 ${alternativeGate.density}%에 불과한 게이트 ${altLetter}(으)로 입장하시는 것을 적극 권장합니다. 훨씬 수월하게 입장할 수 있습니다.`,
        hi: `गेट ${mentionedGate} वर्तमान में अत्यधिक व्यस्त है (${currentDensity}% क्षमता)। हम इसके बजाय गेट ${altLetter} का उपयोग करने की सलाह देते हैं, जहाँ भीड़ केवल ${alternativeGate.density}% है। यह आपको कम समय में आपकी सीट तक पहुँचाएगा।`
      };
      ...
      if (diff > 30) {
        ...
        const responses: Record<string, string> = {
          ...
          ar: `رغم أن البوابة ${mentionedGate} مزدحمة بشكل متوسط (${currentDensity}%)، إلا أن البوابة ${altLetter} أكثر سلاسة بكثير حيث تبلغ سعتها (${bestGateObj.density}%). ننصح بالمرور عبر البوابة ${altLetter} لضمان الدخول السريع وقبل إغلاق التفتيش.`,
          ja: `ゲート${mentionedGate}は中程度の混雑（混雑度${currentDensity}%）ですが、ゲート${altLetter}の方が大幅に空いています（混雑度${bestGateObj.density}%）。スムーズな入場のためにゲート${altLetter}への迂回をお勧めします。`,
          zh: `虽然 ${mentionedGate} 号门目前拥堵程度中等（约 ${currentDensity}%），但 ${altLetter} 号门明显更加顺畅（仅 ${bestGateObj.density}%）。为了确保您能迅速入场，建议您改走 ${altLetter} 号门。`,
          ko: `게이트 ${mentionedGate}은 보통의 혼잡도(${currentDensity}%)를 보이고 있으나, 게이트 ${altLetter}이 훨씬 여유롭습니다(${bestGateObj.density}%). 원활한 입장을 위해 게이트 ${altLetter} 이용을 추천합니다.`,
          hi: `यद्यपि गेट ${mentionedGate} मध्यम रूप से व्यस्त (${currentDensity}%) है, गेट ${altLetter} काफी खाली (${bestGateObj.density}%) है। त्वरित प्रवेश के लिए हम आपको गेट ${altLetter} से जाने की सलाह देते हैं।`
        };
        finalResponseInLanguage = responses[langCode] || responses.en;
      } else {
    ...
    const responses: Record<string, string> = {
      ...
      ar: `بما أنه لم يتم تحديد بوابة بداية في استفساركم، فإننا ننصح بشدة بالدخول عبر البوابة ${bestGateLetter}. تعد الأقل ازدحاماً حالياً (سعة ${bestGateObj.density}%)، مما يضمن لكم حداً أدنى من الانتظار والوصول المباشر إلى مقعدكم.`,
      ja: `クエリに開始ゲートが指定されていなかったため、現在最も混雑度が低い（わずか${bestGateObj.density}%）ゲート${bestGateLetter}からのご入場を強くお勧めします。待ち時間を最短に抑えられます。`,
      zh: `由于您未在咨询中提供起始通道，我们极力推荐您走 ${bestGateLetter} 号门。这里目前是全场最顺畅的通道（仅 ${bestGateObj.density}% 拥堵），能保证您最快进场。`,
      ko: `문의사항에 출발 게이트가 명시되지 않아, 현재 가장 여유로운 게이트 ${bestGateLetter}(혼잡도 ${bestGateObj.density}%)을 통한 입장을 강력 추천합니다. 대기 시간을 최소화하여 더 편안하게 입장하실 수 있습니다.`,
      hi: `चूंकि आपके प्रश्न में कोई गेट निर्दिष्ट नहीं था, इसलिए हम गेट ${bestGateLetter} से जाने की सलाह देते हैं। यह वर्तमान में सबसे खाली प्रवेश द्वार (${bestGateObj.density}% क्षमता) है, जिससे न्यूनतम प्रतीक्षा समय लगेगा।`
    };
    finalResponseInLanguage = responses[langCode] || responses.en;
  }
  ...
  if (isAccessible) {
    const note = accessibilityNotes[langCode] || accessibilityNotes.en;
    finalResponse = `${finalResponse}\n\n${note}`;
  }
  
  if (isSustainable) {
    const note = sustainabilityNotes[langCode] || sustainabilityNotes.en;
    finalResponse = `${finalResponse}\n\n${note}`;
  }
